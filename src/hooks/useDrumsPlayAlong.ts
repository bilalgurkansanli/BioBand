...
export function useDrumsPlayAlong(
  ...
  const buildSession = useCallback((): ResolvedDrumSession | null => {
    ...
    const session = resolveDrumPlaySession(song, scope);
  ...
  }, [buildSession, clearTimers, finishDemo, playHit]);
  ...
  const handlePadPress = useCallback(
    (padId: DrumSoundId) => {
      ...
      if (padId === expected.padId && delta <= HIT_WINDOW_MS) {
        playHit(padId);
        statsRef.current.hits += 1;
        advancePointer();
      } else if (padId === expected.padId) {
        playHit(padId);
        statsRef.current.wrongPresses += 1;
      } else {
        playHit(padId);
    ...
    [advancePointer, getElapsedMs, playHit],
  );
